...
class AG{
	...
	AG(double alpha,GraphCost *_g, Instance &_instance, unsigned _n, unsigned _p, double _pe, double _pm, double _rhoe, unsigned _K, unsigned _MAXT, long unsigned _rngSeed, unsigned _generation, unsigned _X_INTVL, unsigned _X_NUMBER, unsigned _MAX_GENS):instance(_instance){
	...
	decoder = new SampleDecoder(_instance, checador, ls);
	...
	std::tuple <std::vector<int>,int>  solve(){
		...
		//return make_pair(Solution,(int)ls.get_cost());
